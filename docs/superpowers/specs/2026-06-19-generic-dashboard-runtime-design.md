...
- **Versioning & artifacts:** MIDL is versioned `MAJOR.MINOR.BUILD` (the
  firmware CI scheme): same-major with `minor` forward-compatible (old
  config runs on a newer build), `major` breaking. The MIDL bundle is a
  **per-board build/release artifact** and **static content on the
  board**; the manager carries **migrations** to convert configs across
  versions (§3.8).
...
### 3.8 MIDL versioning & artifacts

**Version = `MAJOR.MINOR.BUILD`** (the same scheme as the firmware CI
release version), carried by both the capabilities manifest and every
config document:
- **MAJOR** — breaking. Cross-major documents are **incompatible** and
  must be migrated before they apply.
- **MINOR** — additive, **forward-compatible**: a config authored at an
  older minor runs on a newer build of the same major ("old config works
  on new build"). A config that uses newer-minor features is rejected on
  an older build — the expected asymmetry.
- **BUILD** — identity only; no compatibility meaning; pins the exact
  artifact.

Apply-time compatibility (config `C`, device `D`, same resolution class):
```
compatible(C, D) := C.midl.major == D.midl.major
                 && C.midl.minor <= D.midl.minor      # old config on newer build
```
otherwise the manager migrates `C` to `D`'s version first.

**The bump is derived from the catalog diff** (so the generator sets it
mechanically): an additive change (new element / attr / preset / source
kind, loosened limit) bumps **MINOR**; a removal / rename / semantic
change bumps **MAJOR**; a no-catalog-change rebuild bumps **BUILD**.

**Artifacts.** The MIDL bundle — `{ capabilities manifest, config +
manifest JSON Schemas, generated TS/JS validator + bindings, version
migrations }` — is produced **per board variant** at build time and:
- published as a **CI build artifact** and attached to **release
  artifacts**, versioned `MAJOR.MINOR.BUILD` (every release pins its
  exact MIDL);
- **embedded as static content on the board** and served over HTTP (the
  device-served standalone validator, §3.6);
- associated **per config** — each document records the MIDL version it
  targets, so the right bundle is retrieved for validation or migration.

**Migration lives in the manager (TS/JS).** The manager carries
**migrations** that convert a config between MIDL versions — **required
across majors** (hand-authored per major step, shipped and versioned with
the bundle), best-effort across minors (down-levelling may drop
newer-minor features, with a warning). So an old config from the SignalK
historical library (§3.7) or an imported file can be migrated to a target
device's MIDL major before it is applied. Migrations are pinned by the
golden corpus: a document at version X migrated to Y must validate
against Y's manifest.
...
| 4 | Single **C++ element catalog** (source of truth) + build-time **capabilities generator** → per-class MIDL manifest versioned **`MAJOR.MINOR.BUILD`** (bump derived from catalog diff), published as **CI/release artifact** + embedded on board, served verbatim at `/api/diag`; dispatch-covers-catalog test | `src/`(catalog), `capabilities.cpp`, `tools/`, CI |
| 5 | **YB-MIDL schemas + MIDL→TS/JS bindings generator + satisfaction validator** + golden corpus, shared by manager, web & host tests; **device-served static validator bundle** (HTTP asset) for plugin-less direct authoring; **version migrations** (per-major, corpus-tested) | `schemas/`, `tools/`, `test/test_midl`, `web.cpp` |
...
- **YB-MIDL conformance** (host + CI): the generated manifest validates
  ...
  case.
- **Version compatibility & migration** (host + CI): `compatible(C, D)`
  matches the major/minor rule; the catalog-diff version bump is correct
  (additive → minor, breaking → major); a config at version X migrated to
  Y validates against Y's manifest.
