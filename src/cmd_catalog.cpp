...
static const Entry CATALOG[] = {
    ...
    {"net", "id <name>",                    "Set device id (BLE/mDNS/OTA hostname); reboots", true, true},
    ...
    {"signalk", "sk-ap-state <standby|auto|wind|track>", "PUT autopilot state via SK REST", true, true},
    ...
    {"nmea-wifi", "nmea-wifi tcp <host> <port>", "Configure TCP-client mode and enable", true, true},
    ...
    {"boat", "boat timeout <n2k|wifi|sk|demo> <ms>", "Tune freshness window at runtime", true, true},
    ...
    {"input-test", "tap <x> <y> [hold_ms]", "Synth tap: press, hold, release (LVGL CLICKED)", false, true},
...
const Entry *entries() { return CATALOG; }
size_t entry_count() { return sizeof(CATALOG) / sizeof(CATALOG[0]); }
