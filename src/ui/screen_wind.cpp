...
static void build_bezel(lv_obj_t *parent) {
    ...
    // White band + green rail (reference rim), then the structural rings.
    make_ring_at(bezel, bcx, bcy, R_BEZEL * 2, 16, theme.arc_band, LV_OPA_90);
    make_ring_at(bezel, bcx, bcy, R_BEZEL * 2 + 12, 4, theme.good, LV_OPA_80);
    make_ring_at(bezel, bcx, bcy, R_BEZEL * 2 - 16, 1, 0x0c1828);  // inner highlight
...
static int16_t deg_to_lvgl(double deg) {
    // LVGL transform_rotation is in 0.1° units, [0..3600). Quantize to whole
    // degrees - sub-degree sensor jitter would otherwise force a re-invalidate
    // of large transformed bounding boxes on every refresh.
