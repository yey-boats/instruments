// Host harness that renders the REAL fullscreen wind dial (src/ui/screen_wind.cpp)
...
int main(int argc, char **argv) {
    ...
    lv_obj_t *root = ui::wind::build(lv_screen_active());
    if (!root) {
        fprintf(stderr, "ui::wind::build() returned null\n");
        return 1;
    }

    // A few refresh ticks so the dirty-value caches latch the stub snapshot and
    // the markers rotate to AWA / TWA / set.
    for (int i = 0; i < 8; ++i) {
        ui::wind::refresh();
        lv_tick_inc(20);
        lv_timer_handler();
